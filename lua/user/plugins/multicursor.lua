...
return {
	...
	config = function()
		...
		end, desc("Multicursor add cursor and jump to next search result"))

		-- Search add all
		set("n", "<leader>S", mc.searchAllAddCursors, desc("Multicursor add cursor for all search results"))

		-- Match add cursor
		set("x", "<leader>D", function()
			mc.matchAddCursor(1)
		end, desc("Multicursor add cursor for match and go to next"))
		...
		set(
			...
			"<leader>I",
			mc.insertVisual,
			desc("Multicursor create a cursor for each line of the visual selection, and enter insert mode")
		)
		...
		mc.addKeymapLayer(function(layer_set)
			...
			-- Skip line
			layer_set("n", "<Up>", function()
				mc.lineSkipCursor(-1, { skipEmpty = false })
			end, desc("Multicursor skip line above"))
			layer_set("n", "<Down>", function()
				mc.lineSkipCursor(1, { skipEmpty = false })
			end, desc("Multicursor skip line below"))

			-- Skip match
			layer_set("x", "<Up>", function()
				mc.matchSkipCursor(-1)
			end, desc("Multicursor skip and go to previous match"))
			layer_set("x", "<Down>", function()
				mc.matchSkipCursor(1)
			end, desc("Multicursor skip and go to next match"))
			...
			-- Search
			layer_set("n", "<C-m>", function()
				mc.searchAddCursor(-1)
			end, desc("Multicursor add cursor and jump to previous search result"))
			layer_set("n", "<C-n>", function()
				mc.searchAddCursor(1)
			end, desc("Multicursor add cursor and jump to next search result"))
			layer_set("n", "N", function()
				mc.searchSkipCursor(-1)
			end, desc("Multicursor jump to the previous result without adding a cursor"))
			layer_set("n", "n", function()
				mc.searchSkipCursor(1)
			end, desc("Multicursor jump to the next result without adding a cursor"))

			-- Match
			layer_set("x", "<C-e>", function()
				mc.matchAddCursor(-1)
			end, desc("Multicursor add cursor for match and go to previous"))
			layer_set("x", "<C-d>", function()
				mc.matchAddCursor(1)
			end, desc("Multicursor add cursor for match and go to next"))
			...
			layer_set(
				...
				"M",
				mc.matchCursors,
				desc("Multicursor match new cursors within visual selections by regex")
			)
		end)
