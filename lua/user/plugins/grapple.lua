	...
	config = function(_, opts)
		...
		local function next()
			if trouble.is_open() then
				...
				if vim.iter(win_infos):any(function(w)
					return w.loclist ~= 0
				end) then
				...
				elseif vim.iter(win_infos):any(function(w)
					return w.quickfix ~= 0
				end) then
					local ok = pcall(function()
						vim.cmd("cnext")
		...
		vim.keymap.set("n", "<M-2>", function()
			grapple.cycle_tags("next", { index = 1 })
		end, desc("Grapple file 2"))
		vim.keymap.set("n", "<M-3>", function()
			grapple.cycle_tags("next", { index = 2 })
		end, desc("Grapple file 3"))
		vim.keymap.set("n", "<M-4>", function()
			grapple.cycle_tags("next", { index = 3 })
		end, desc("Grapple file 4"))
		vim.keymap.set("n", "<M-5>", function()
			grapple.cycle_tags("next", { index = 4 })
		end, desc("Grapple file 5"))
		vim.keymap.set("n", "<leader>hp", function()
			grapple.tag({ index = 1 })
		end, desc("Grapple prepend file"))
		vim.keymap.set("n", "<leader>h1", function()
			grapple.tag({ index = 1 })
		end, desc("Grapple add file index 1"))
		vim.keymap.set("n", "<leader>h2", function()
			grapple.tag({ index = 2 })
		end, desc("Grapple add file index 2"))
		vim.keymap.set("n", "<leader>h3", function()
			grapple.tag({ index = 3 })
		end, desc("Grapple add file index 3"))
		vim.keymap.set("n", "<leader>h4", function()
			grapple.tag({ index = 4 })
		end, desc("Grapple add file index 4"))
		vim.keymap.set("n", "<leader>h5", function()
			grapple.tag({ index = 5 })
		end, desc("Grapple add file index 5"))
		vim.keymap.set("n", "<leader>ha", function()
			grapple.tag()
		end, desc("Grapple append file"))
		vim.keymap.set("n", "<leader>he", function()
			grapple.toggle_tags()
		end, desc("Grapple edit list"))
