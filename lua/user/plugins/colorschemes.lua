...
for _, cs in ipairs(colorschemes) do
	...
	local is_current = (
		current_colorscheme:sub(1, #cs.name) == cs.name
		or cs.variants and vim.tbl_contains(cs.variants, current_colorscheme)
	)
	...
	callback = function(args)
		...
		if setup == nil then
			for key, value in pairs(name2setup) do
				if name:sub(1, #key) == key or vim.tbl_contains(name2variants[key], name) then
					name = key
					setup = value
					break
				end
		...
		if (type(disable_italics) == "table" and vim.tbl_contains(disable_italics, name)) or disable_italics then
